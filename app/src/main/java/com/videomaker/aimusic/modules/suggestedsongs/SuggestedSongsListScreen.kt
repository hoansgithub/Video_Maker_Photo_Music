...
fun SuggestedSongsListScreen(
    viewModel: SuggestedSongsListViewModel,
    onNavigateBack: () -> Unit,
    ...
    LaunchedEffect(navigationEvent) {
        navigationEvent?.let { event ->
            when (event) {
                is SuggestedSongsNavigationEvent.NavigateBack -> onNavigateBack()
                is SuggestedSongsNavigationEvent.NavigateToAssetPicker -> {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.songs_suggest_for_you)) },
                navigationIcon = {
        ...
        when (val state = uiState) {
            is SuggestedSongsListUiState.Loading -> {
